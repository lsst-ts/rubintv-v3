...
from rubintv.config.models import (
    AdminRedisMenu,
    AdminRedisMenuItem,
    Camera,
    Channel,
...
def _global_metadata_columns(
    ...
    for camera_name, columns in raw.items():
        if not isinstance(columns, dict):
            raise ConfigError(
                f"metadata_columns for {camera_name!r} must be a mapping"
            )
...
def _site_locations(
    ...
    if site not in bucket_configurations:
        raise ConfigError(
            f"unknown site {site!r}; declared sites: "
            f"{sorted(bucket_configurations)}"
        )
